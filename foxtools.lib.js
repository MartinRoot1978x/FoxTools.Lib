...
(function (foxtools) {
    /**
     * Классы для работы с хитрым API v2.0
     */
    var lib;
    (function (lib) {
        /**
        ...
        lib.requestProcess = requestProcess;
        ...
        var baseResult = (function () {
            ...
            baseResult.prototype.addTrace = function (message, elepsedTime) {
                ...
                console.log(foxtools.utility.format('TRACE: {0} - {1}', [message, elepsedTime]));
            };
            baseResult.prototype.addMessage = function (type, text) {
                this.messages.push(new messageItem(type, text));
                console.log(foxtools.utility.format('{0}: {1}', [type, text]));
            };
            return baseResult;
        })();
        lib.baseResult = baseResult;
        /**
         * Клиент для работы с хитрым API v2.0.
         */
        var client = (function () {
            function client() {
            }
             ...
             */
            client.getToken = function (url, options) {
                var token = null;
            ...
            /**
             * Делает указанную форму хитрой.
             *
             * @param selector Имя элемента или объект JQuery.
             * @param options Дополнительные параметры.
             */
            client.foxyForm = function (selector, options) {
        ...
        var formRequest = (function (_super) {
            ...
            formRequest.prototype.submit = function () {
                ...
                var token = foxtoolsToken.parse($request.token);
                ...
                if (token !== null && token.url !== null && token.value === null) {
                    foxtools.lib.client.getToken(token.url, {
            ...
            formRequest.prototype.justSubmit = function (token) {
                ...
                if (type.toUpperCase() == 'POST') {
                    data = new window['FormData'](form); //TypeScript bug --new FormData(form);
                }
                else {
                    data = $form.serialize();
                }
                ya.hit($form.attr('action'), methodName);
                $.ajax({
                    type: type,
        ...
        lib.formRequest = formRequest;
        /** Представляет хитрый запрос к хитрому API. */
        var request = (function (_super) {
            __extends(request, _super);
            /**
             * Инициализирует новый экземпляр хитрого запроса.
             *
             * @param methodName Название метода, который необходимо выполнить.
             */
            ...
            request.prototype.justExecute = function (token) {
                ...
                ya.hit($request.endpoint, $request.methodName);
                $.ajax({
        ...
        lib.httpParameterCollection = httpParameterCollection;
        ...
        lib.httpParameter = httpParameter;
        ...
        var stringArrayResult = (function (_super) {
            ...
            /** Возвращает строковое значение результата выполнения запроса к API. */
            stringArrayResult.prototype.toString = function () {
                return this.items.join('\n');
            };
            return stringArrayResult;
        })(baseResult);
        lib.stringArrayResult = stringArrayResult;
        ...
        lib.countryResult = countryResult;
        ...
        var foxtoolsToken = (function () {
             ...
             */
            foxtoolsToken.parse = function (token) {
        ...
        lib.foxtoolsToken = foxtoolsToken;
        /** Хитрый класс работы с Яндекс.Метрика. */
        var ya = (function () {
            function ya() {
            }
            /**
             ...
             */
            ya.hit = function (url, title) {
                try {
                    //utility.initYa();
                    if (ya.counter !== undefined && ya.counter !== null) {
                        ya.counter.hit(url, title, null);
                    }
                }
                catch (ex) {
                }
                ...
                try {
                    ...
                    if (window['Ya'] === undefined || window['Ya'] === null) {
                        ...
                        $.getScript('//mc.yandex.ru/metrika/watch.js', function () {
                            ya.counter = new window['Ya'].Metrika({ id: 24654338, enableAll: true });
                        });
                    }
                    else {
                        if (ya.counter === undefined || ya.counter === null) {
        ...
        })(lib.dataMode || (lib.dataMode = {}));
        var dataMode = lib.dataMode;
        /** Перечень режимов обработки данных. */
        (function (outputFormat) {
            outputFormat[outputFormat["text"] = 0] = "text";
            outputFormat[outputFormat["file"] = 1] = "file";
        })(lib.outputFormat || (lib.outputFormat = {}));
        var outputFormat = lib.outputFormat;
        ...
        })(lib.errorCode || (lib.errorCode = {}));
        var errorCode = lib.errorCode;
        ;
        ...
        })(lib.yesNoAny || (lib.yesNoAny = {}));
        var yesNoAny = lib.yesNoAny;
    })(lib = foxtools.lib || (foxtools.lib = {}));
    ...
    var utility = (function () {
         ...
         */
        utility.replace = function (str, find, repl) {
        ...
        /**
         * Получае значение указанного параметра из строки запроса.
         *
         * @param name Имя параметра, значение которого следует получить.
         * @param queryString Строка параметров запроса, из которой следует извлечь указанный параметр. По умолчанию используйте текущая строка (location.search).
         */
        utility.getQueryStringParameterByName = function (name, queryString) {
        ...
        /**
         * Проверяет, является указанная строка URL или нет.
         */
        utility.isUrl = function (value) {
            return new RegExp('^(http|https):\/\/([^\x20\r\n]*)$', 'i').test(value);
        };
        return utility;
    })();
    foxtools.utility = utility;
    /**
     * Вспомогательный метод для создания хитрой формы.
     *
     * @param selector Имя элемента или объект JQuery.
     * @param options Дополнительные параметры.
     */
    function apiForm(selector, options) {
        foxtools.lib.client.foxyForm(selector, options);
    }
    foxtools.apiForm = apiForm;
    /**
     * Вспомогательный для создания хитрого запроса.
     *
     * @param methodName Название метода, который необходимо выполнить.
     */
    function apiRequest(methodName) {
        return new foxtools.lib.request(methodName);
    }
    foxtools.apiRequest = apiRequest;
})(foxtools || (foxtools = {}));
// альтернатива $(document).ready
if (document.addEventListener) {
    document.addEventListener('DOMContentLoaded', foxtools.lib.client.ready);
}
else if (document.attachEvent) {
    window.attachEvent("onload", foxtools.lib.client.ready);
}
